...
type FilmCardProps = Film & { className?: string; children?: React.ReactNode; onClick?: (id: number) => void };
...
export function FilmCard({
    ...
    const [rateMovie] = filmsApi.useRateMovieMutation()
    return (
        <Card className={`${className} film-card`} onClick={() => onClick && onClick(id)}>
            <div className='film-card-layout'>
                ...
                <h2>{title}</h2>
                {
                    user.isLogin && <Rating rating={getCurrentRating(user.username, id)} onChange={(rating) => {
